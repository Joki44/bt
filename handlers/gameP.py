...
async def start_game(call: types.CallbackQuery):
    ...
    for player in table["players"]:
        ...
        if player["tele_id"] == int(carts):
            ...
            msg = await bot.send_message(player['tele_id'],
                ...
                reply_markup = InlineKeyboardMarkup().add(
                    InlineKeyboardButton(
                        text = "50",
                        callback_data = f"bpublicet|{tableId}|50|50"
                    ),
                    InlineKeyboardButton(
                        text = "100", 
                        callback_data = f"bpublicet|{tableId}|100|100"
                    ),
                )
            )
            player["msg"] = msg
            ...
            loop.create_task(timeIsUp(msg, tableId, surplus_ceiling, table["carts"]))
...
async def set_bet(call:types.CallbackQuery):
    ...
    if call.data.split('|')[2] != 'equal':
        surplus_bet = int(call.data.split('|')[2])
        old_bet = int(call.data.split('|')[3])
        suma = old_bet + surplus_bet - table["players"][table["carts"]]['bet'] # будет использоваться при запросах в бд
        with connection.cursor() as cursor:
            ...
            if player["tele_id"] == int(carts):
                ...
                msg = await bot.send_message(player['tele_id'],
                    ...
                    reply_markup = InlineKeyboardMarkup().add(
                        InlineKeyboardButton(
                            text = "Уровнять",
                            callback_data = f"bpublicet|{tableId}|equal|{old_bet}"
                        ),
                        InlineKeyboardButton(
                            text = "50",
                            callback_data = f"bpublicet|{tableId}|50|{old_bet}"
                        ),
                        InlineKeyboardButton(
                            text = "100", 
                            callback_data = f"bpublicet|{tableId}|100|{old_bet}"
                        )
                    )
                )  
                player["msg"] = msg
                ...
                table["players"][table["carts"]]["the_move_is_made"] = False
                loop.create_task(timeIsUp(msg, tableId, old_bet, table["carts"]))
    ...
    if equal == True:
        ...
        for player in players:
            ...
            if player["tele_id"] == int(carts):
                msg = await bot.send_message(player['tele_id'],
                    ...
                    reply_markup = InlineKeyboardMarkup().add(
                        InlineKeyboardButton(
                            ...
                            callback_data = f"capublicrd|{tableId}|{player['cards'][0]}"
                        ),
                        InlineKeyboardButton(
                            text = f"{PLAYING_CARDS_BTN_VALUE[player['cards'][1]]}",
                            callback_data = f"capublicrd|{tableId}|{player['cards'][1]}"
                        ),
                        InlineKeyboardButton(
                            text = f"{PLAYING_CARDS_BTN_VALUE[player['cards'][2]]}",
                            callback_data = f"capublicrd|{tableId}|{player['cards'][2]}"
                        )
                    )
                )
                player["msg"] = msg
                ...
                table["players"][table["carts"]]["the_move_is_made"] = False
                loop.create_task(timeIsUp(msg, tableId, 0, table["carts"]))
    ...
    if len(players) <= table["turns"]:
        ...
        for player in players:
            if player['thrownCard'] == vin:
                ...
                message_vin = f"\nИгрок {player['userNmae']} выйграл взятку"
                ...
                if len(cardss) == 2:
                    markup = InlineKeyboardMarkup().add(
                        InlineKeyboardButton(
                            ...
                            callback_data = f"capublicrd|{tableId}|{cardss[0]}"
                        ),
                        InlineKeyboardButton(
                            text = f"{PLAYING_CARDS_BTN_VALUE[cardss[1]]}",
                            callback_data = f"capublicrd|{tableId}|{cardss[1]}"
                        )
                    )
                    ...
                    if player['azi'] == 1:
                        ...
                        msg = await bot.send_message(player['tele_id'],
                            ...
                            reply_markup = InlineKeyboardMarkup().add(
                                InlineKeyboardButton(
                                    text = f"Да",
                                    callback_data = f"apubliczi|{tableId}|yes|1"
                                ),
                                InlineKeyboardButton(
                                    text = f"Нет",
                                    callback_data = f"apubliczi|{tableId}|not"
                                )
                            )
                        )
                    if player['azi'] == 2:
                        msg = await bot.send_message(player['tele_id'],
                            ...
                            reply_markup = InlineKeyboardMarkup().add(
                                InlineKeyboardButton(
                                    text = f"Да",
                                    callback_data = f"apubliczi|{tableId}|yes|2"
                                ),
                                InlineKeyboardButton(
                                    text = f"Нет",
                                    callback_data = f"apubliczi|{tableId}|not"
...
def game_handlers(dp: Dispatcher):
    dp.register_callback_query_handler(start_game, text_contains="start_public_game" )
    # dp.register_callback_query_handler(set_ceiling, text_contains="ceilpublicing" )
    dp.register_callback_query_handler(set_bet, text_contains="bpublicet" )
    dp.register_callback_query_handler(card_game, text_contains="capublicrd" )
    dp.register_callback_query_handler(azi, text_contains="apubliczi" )
