...
async def timeIsUp(message, tableId, surplus_ceiling, carts):
    table = games[tableId]
    ...
    if table["players"][carts]["the_move_is_made"] == False:
                    ...
                    if player["tele_id"] == int(carts):
                        ...
                        msg = await bot.send_message(player['tele_id'],
                            ...
                            reply_markup = InlineKeyboardMarkup().add(
                                ...
                                InlineKeyboardButton(
                                    text = "Сдать", 
                                    callback_data = f"bet|{tableId}|stop|{old_bet}"
                                )
                            )
                        )  
                        player["msg"] = msg
                    ...
                    if player["tele_id"] == int(carts):
                        msg = await bot.send_message(player['tele_id'],
                            ...
                            reply_markup = InlineKeyboardMarkup().add(
                                ...
                                InlineKeyboardButton(
                                    text = "Сдать", 
                                    callback_data = f"card|{tableId}|stop"
                                )
                            )
                        )
                        player["msg"] = msg
        ...
        if table["namePartGame"] == 2:
            ...
            if table["partGame"] < 3:
                for player in players:
                    if len(player['cards']) == 3:
                        text = f"Ваши карты: {PLAYING_CARDS_BTN_VALUE[player['cards'][0]]} {PLAYING_CARDS_BTN_VALUE[player['cards'][1]]} {PLAYING_CARDS_BTN_VALUE[player['cards'][2]]}" + strCardsOnTable
                        markup = InlineKeyboardMarkup().add(
                            InlineKeyboardButton(
                                text = f"{PLAYING_CARDS_BTN_VALUE[player['cards'][0]]}",
                                callback_data = f"card|{tableId}|{player['cards'][0]}"
                    ...
                    if len(player['cards']) == 2:
                        text = f"Ваши карты: {PLAYING_CARDS_BTN_VALUE[player['cards'][0]]} {PLAYING_CARDS_BTN_VALUE[player['cards'][1]]}" + strCardsOnTable
                        markup = InlineKeyboardMarkup().add(
                            InlineKeyboardButton(
                                text = f"{PLAYING_CARDS_BTN_VALUE[player['cards'][0]]}",
                                callback_data = f"card|{tableId}|{player['cards'][0]}"
                    ...
                    if len(player['cards']) == 1:
                        text = f"Ваши карты: {PLAYING_CARDS_BTN_VALUE[player['cards'][0]]}" + strCardsOnTable
                        markup = InlineKeyboardMarkup().add(
                            InlineKeyboardButton(
                                text = f"{PLAYING_CARDS_BTN_VALUE[player['cards'][0]]}",
                                callback_data = f"card|{tableId}|{player['cards'][0]}"
                    ...
                    if len(player['cards']) == 0:
                        ...
                        if player["tele_id"] != int(carts):
                            await bot.send_message(player['tele_id'],
                                f"На кону: {ceiling}\
                                \nКозырь: {trump}\n"+strCardsOnTable+message_vin+out+"\nОжидаем хода другого игрока"+out,
                            )   
            ...
            if table["partGame"] == 3:
                ...
                if bigestBribe > 1 :
                    ...
                    for player in table["players"]:
                        if player["tele_id"] == int(carts):
                            await bot.send_message(player['tele_id'],
                                ...
                                reply_markup = InlineKeyboardMarkup().add(
                                    InlineKeyboardButton(
                                        text = "50",
                                        callback_data = f"ceiling|{tableId}|50"
                                    ),
                                    InlineKeyboardButton(
                                        text = "100", 
                                        callback_data = f"ceiling|{tableId}|100"
                                    )
                                ).add(
                                    InlineKeyboardButton(
                                        text = "Сдать", 
                                        callback_data = f"ceiling|{tableId}|stop"
                                    )
                                )
                            )
                        ...
                        if player["tele_id"] != int(carts):
                            await bot.send_message(player['tele_id'],
                                f'Ожидаем ставики другого игрока',
                            )
